...
| Streamable HTTP | Sessionless GET/POST `Accept: text/event-stream` → finite SSE (`event: message`). **GET with session** holds SSE + keepalive and queues `tools/list_changed`. JSON POST does **not** drain-and-drop the queue. Discovery JSON adds `sse` / `streamable` / `stdio_live` / `http_url` / `version` / `http_csrf: false` / `catalog_notify` / `listed_tool_count` / `catalog_stale` / `catalog_hint` |
