...
| Resources (15) | `gsv://vision/{manifest,feed,extensions}` · `gsv://docs/{mcp-openbot,handoff,next,fingerprints,post-always-on,rust-dev,omni-catalog,settings-telegram,solo-squad-jail,ranks,rules-check,vdc}` — allowlist + `preview::resolve`; unknown / `file://` / `..` → JSON-RPC `-32602` |
