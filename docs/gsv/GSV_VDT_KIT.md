...
2. **Cursor відкривати на GSV.** `абракадабра` / `abrakadabra` / `agi` **спочатку** сканує environment
...
Наступна сесія: **File → Open Folder → `S:\rust\GSV`**, потім у чат: `абракадабра`, `abrakadabra`, або `agi`.
