...
| GET | `/mcp` | MCP discovery (`gsv_mcp_openbot` + `sandbox` GSV crate path + 56 tools + 13 resources + 3 prompts + `stdio`/`stdio_live`/`http`/`http_url`/`version`/`http_csrf`/`tool_count`/`resource_count`/`prompt_count`/`logging`/`completions`/`log_level`/`subscribe`/`subscription_count`/`sse`/`streamable`/`sessions`/`session_count`/`tools_list_changed`/`catalog_notify`/`listed_tool_count`/`session_listed`/`catalog_stale`/`catalog_hint`); sessionless `Accept: text/event-stream` flushes pending notifications as finite SSE; **GET with `Mcp-Session-Id` holds** the stream and queues `tools/list_changed`; unknown `Mcp-Session-Id` → 404 |
