...
| GET | `/api/health` | health-чек (`crate_version`, `version_lag`, `watchdog_alive`, `disk_ok` / `disk_violation` — process `ok` stays true on S0 disk trip; `update_available` matches Update box; `keep_live` merge of the always-on kit report — see `/api/keep-live`) |
