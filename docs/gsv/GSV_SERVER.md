...
| GET | `/mcp` | MCP discovery (`gsv_mcp_openbot` name + tool list) |
...
- Перевіряє core boxes (`/api/health`, `/api/tracker`, `/api/sli`, `/api/toolchain`, `/api/update`, `/api/ratio`, `/api/omni/status`), усі `/api/vision*` (ok-гейт), SVG-ассети та **усі 31 зареєстрованих карток** `/api/ui/card/:name` (non-empty `html`).
