...
| GET | `/mcp` | MCP discovery (`gsv_mcp_openbot` name + 26 tools + 6 resources + 3 prompts + `stdio`/`http`/`tool_count`/`resource_count`/`prompt_count`) |
| POST | `/mcp` | MCP JSON-RPC (initialize / tools/* / resources/* / prompts/*); stdio twin is `gsv-mcp` |
