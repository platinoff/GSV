...
| GET | `/api/telegram` | Godfather bind status (`ok`, `channel_id`, `token_set`, `bot_username`, `chat_title`, `member_count`, `chat_kind`, `last_probe`, `polling`, `poll_alive`, `last_poll_ts`, `last_ingest_kind`, `last_bus_ts`, `last_bus_error`; never `bot_token`). Live `getChatMemberCount` persists `tickets.member_count` (**band 187**). `X-Telegram-Dry-Run: 1` forces the in-process stub (does not persist). |
...
| POST | `/api/telegram/poll` | One inbound `getUpdates` pass (`ticket`/`hook`/`bus`/`presence`/`claim`/`skip` counts). CSRF; dry-run stub queue in tests. Requires `godfather.poll` or `telegram-relay`. Never `bot_token`. |
...
| POST | `/api/tickets/next` | Standardized next action (`hint` / `tool` / `ticket_id`) from WIP, Godfather hint, or first open row. CSRF. Heartbeats presence. |
| GET | `/api/mds` | Light memory / disk / speed report (`gsv-mds`) |
| GET | `/api/grid` | ALLBGP grid mirror (hub-side durable fleet view): `poolai_alive` + last-known `nodes`/`workers`/`virtual-nodes`/`seats` (kept stale on poolAI failure) + `counts` + history ring (`data/gsv_grid.json`, canon [GSV_VDC.md](GSV_VDC.md)); refresh on the 10 s hub loop (SSE `event: grid` on change) + on demand; env `GSV_POOLAI_URL` default `http://127.0.0.1:8091/api/v1` |
| GET | `/api/apk` | Phone peer disk + settings + WiFi-debug ADB plan (`wifi_debug`, `screenshot:false`). No secrets. Canon [GSV_AGI_PATH.md](GSV_AGI_PATH.md). |
| GET/POST | `/api/edge/{*path}` | Allowlisted reverse proxy onto poolAI `/api/v1/{path}`. Requires `X-Gsv-Edge-Token` or `Authorization: Bearer` matching `GSV_EDGE_TOKEN` / `settings.edge.token`. 20 req/s. 401/404/429/502/503. POST origin `telegram_edge`/`chrome`/`webview` → **403** (phone worker is `apk_edge` only). JSON secrets stripped. CSRF on POST. |
| GET | `/api/health` | health-чек (`crate_version`, `version_lag`, `watchdog_alive`, `disk_ok` / `disk_violation` — process `ok` stays true on S0 disk trip; `update_available` matches Update box; `keep_live` merge of the always-on kit report — see `/api/keep-live`; **band 237** `edge_proxy` status) |
| GET | `/api/keep-live` | always-on kit aggregation (**band 223**): `mode: online\|lan-only` (relay + fresh `telegram_offset.json` ≤90 s ⇒ online; cable-cut ⇒ `lan-only`, offline signal **2026-09-14**) + `hint` (`… · {mode}`) + `KeepLiveReport { gsv, telenetis, llama_rs, omniroute }` each `{ alive, url, version?, lag?, latency_ms?, uptime_secs? }` (**band 226**); 1s probes, fail-open — `ok` stays true when a peer is down; env overrides `GSV_KEEP_LIVE_GSV_URL` / `GSV_KEEP_LIVE_TELENETIS_URL` / `OMNIROUTE_URL` / `LLAMA_HEARTBEAT_PATH`; omniroute probe URL chain `OMNIROUTE_URL` → `GSV_KEEP_LIVE_OMNIROUTE_URL` → `GSV_OMNIROUTE_URL` → `http://127.0.0.1:20128` (**band 227**) |
| GET | `/mcp` | MCP discovery (`gsv_mcp_openbot` + `sandbox` GSV crate path + 59 tools + 15 resources + 3 prompts + `stdio`/`stdio_live`/`http`/`http_url`/`version`/`http_csrf`/`tool_count`/`resource_count`/`prompt_count`/`logging`/`completions`/`log_level`/`subscribe`/`subscription_count`/`sse`/`streamable`/`sessions`/`session_count`/`tools_list_changed`/`catalog_notify`/`listed_tool_count`/`session_listed`/`catalog_stale`/`catalog_hint`/`keep_live_summary`); sessionless `Accept: text/event-stream` flushes pending notifications as finite SSE; **GET with `Mcp-Session-Id` holds** the stream and queues `tools/list_changed`; unknown `Mcp-Session-Id` → 404 |
...
- Env: `GSV_BASE_URL` / `GSV_KEEP_LIVE_TELENETIS_URL` / `LLAMA_HEARTBEAT_PATH` / `OMNIROUTE_URL` / `GSV_OMNIROUTE_URL` (omniroute URL chain, **band 227**: `OMNIROUTE_URL` → `GSV_KEEP_LIVE_OMNIROUTE_URL` → `GSV_OMNIROUTE_URL` → default `http://127.0.0.1:20128`).
...
**Always-on (band 144 + watchdog + band 153 rust live):** run a live copy (`target/live/gsv-server.exe` via `cargo xtask live`) so `cargo test`/`build` does not lock the listening process. `POST /api/update/apply` emits SSE `offline` and exits (gated by `GSV_UPDATE_APPLY_EXIT`); the supervisor recopies debug → live and rebinds `:9999`; the page stays **offline** until SSE `onopen` then resyncs. If the supervisor process dies (Cursor abort), `gsv-watchdog` probes `/api/health` and respawns the live copy. When **gsv-server** debug is newer than a healthy live copy (or health `version_lag`), the watchdog POSTs `/api/update/apply` (lockstep) so Windows can recopy after exit. A stale **watchdog** exe hops via `successor_plan` **each tick** and does not POST apply. Failed apply is heartbeat `lockstep-fail` plus `last_apply_status` / `lockstep_note`. Cooldown while still needed is `lockstep-wait` (never silent `probe-ok`). Oneshot apply uses server-debug-newer **or** health lag, and only yields if the peer pid is still alive and its `bin_version` matches the crate (otherwise it stops the stale peer and takes over). Heartbeat `bin_version` vs crate `version_lag` is on `GET /api/watchdog` (`server_debug_newer` / `watchdog_debug_newer`). `cargo xtask watchdog` copies `gsv-watchdog` to `target/live/` so cargo can overwrite debug. Do **not** kill the live copy before `cargo test`. Spec: [`GSV_ALWAYS_ON_UI.md`](./GSV_ALWAYS_ON_UI.md).
...
**Horizon:** band **196** federated `kind:reclaim` (lease sweep + explicit release post the reclaim; peer boards `in_progress` → `open`; ranks process-local — lifecycle presence → claim → done → reclaim complete). Band **195** federated `kind:done` (the finisher posts the close; host board `in_progress` → `done`; ranks process-local). Band **194** federated `kind:claim` (remote jail claims an open host-board ticket; local `try_dispatch` unchanged). Band **193** federated `kind:presence` (remote jails on host `federation`; local `squad_cap` unchanged). Band **192** ranks (jun-nub L0 · marshal-orchestrator host display) + git console hide. Band **191** channel roles (host/mate/guest/local) + GitHub origin lockstep (`github_ahead` · ticket pick board then `hook github`). Band **187** live Godfather member_count (`getChatMemberCount` · derived `squad_cap`). Band **186** solo/squad/jail. Band **185** Cursor catalog restart lockstep (`catalog_stale` / `catalog_hint` · Galaxy `restart Cursor` — agent refresh does not re-list). Band **184** MCP session catalog lockstep (JSON POST keeps `list_changed` for SSE hold · `listed_tool_count`). Band **183** squad next-action (`gsv_tickets_next`) + MCP `tools/list_changed`. Band **182** MCP-readable Godfather envelopes + Galaxy MCP signal. Band **181** Galaxy glue + S0 disk on health. Band **180** watchdog process lockstep (`hop_successor` each tick). Band **179** Godfather inbound poller. Band **174** solo Telegram tickets (`gsv_telegram_ticket` · `/ticket` ingest). Band **173** vision queue close-lockstep (`bump --band N` = last of N / first of N+1). Band **172** live crate lockstep. Settings/Telegram/tickets **166–171 ✅**. Next drain = **owner pick**. Spec: [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) · [`GSV_OMNI_CATALOG.md`](./GSV_OMNI_CATALOG.md) · [`GSV_RUST_DEV.md`](./GSV_RUST_DEV.md).
