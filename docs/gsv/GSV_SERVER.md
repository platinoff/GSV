...
| GET | `/api/edge` | Hub edge-proxy status (**band 237**): `ok`, `token_set`, `source` (`env`/`file`/`none`), `allowlist`, `rate` (`window_ms`/`cap`), `poolai_base`. Never the token. Canon [GSV_VDC.md](GSV_VDC.md) §6. |
| GET/POST | `/api/edge/{*path}` | Allowlisted reverse proxy onto poolAI `/api/v1/{path}`. Requires `X-Gsv-Edge-Token` or `Authorization: Bearer` matching `GSV_EDGE_TOKEN` / `settings.edge.token`. 20 req/s. 401/404/429/502/503. JSON secrets stripped. CSRF on POST. |
