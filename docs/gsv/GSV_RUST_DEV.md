...
| `.cursor/mcp.json` | Cursor HTTP MCP (`url` → live `:9999/mcp`). Folder **GSV** only — never User (`%USERPROFILE%/.cursor/mcp.json`). Cursor **3.16.29** still uses Streamable HTTP `type: http`; do not Origin-host this kit. |
