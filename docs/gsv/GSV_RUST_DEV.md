...
| APK client | `cargo run --bin gsv-apk` (`--json`, `register --json`, `check-hub URL`, `telegram <kind> --json`, `service-account --json`) — hub `:9999/api/edge` only; Telegram is proxy to APK |
