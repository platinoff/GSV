...
| APK client | `apk/` (`boxes/apk.rs`) | bin `gsv-apk` (`disk` / `register --json` / `check-hub` / `telegram` / `service-account`) | Rust-ratio phone peer: `origin=apk_edge` via `/api/edge` + edge token (not poolAI admin). Never `:8091`. Telegram is proxy-only. |
