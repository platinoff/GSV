...
| **gsv_mcp_openbot** | `mcp.rs` + `gsv-mcp` bin | stdio live copy + `GET`/`POST`/`DELETE /mcp` + Galaxy card `/api/ui/card/mcp` | 40 box tools + 11 `gsv://` resources (band **168** `gsv_tickets` + `gsv_tickets_claim` · **167** `gsv_telegram` · **166** `gsv_settings` + `gsv://docs/settings-telegram` · **164** Cursor 3.16.29 kit lockstep · **160** GSV sandbox + no User MCP · **159** Cursor HTTP url + session SSE hold · **158** live stdio + sync check · **157** omni route) — [`GSV_OMNI_CATALOG.md`](./GSV_OMNI_CATALOG.md) · [`GSV_MCP_OPENBOT.md`](./GSV_MCP_OPENBOT.md) |
...
| Telegram | `telegram/` (`boxes/telegram.rs`) | `GET /api/telegram` | Godfather bind (`getMe`+`getChat`); dry-run stub under cargo test / `X-Telegram-Dry-Run: 1`; poller default off; never `bot_token` — [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) **band 167 ✅** |
