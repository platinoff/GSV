...
| **gsv_mcp_openbot** | `mcp.rs` + `gsv-mcp` bin | stdio + `GET`/`POST`/`DELETE /mcp` + Galaxy card `/api/ui/card/mcp` | 26 box tools + 6 `gsv://` resources (band 137–142 ✅); **band 151 queued:** +products/scan/watchdog/sw/fingerprints → 31 tools + 8 resources — [`GSV_POST_ALWAYS_ON.md`](./GSV_POST_ALWAYS_ON.md) · [`GSV_MCP_OPENBOT.md`](./GSV_MCP_OPENBOT.md) |
