...
| APK client | `apk/` (`boxes/apk.rs`) | bin `gsv-apk` (`disk` / `register --json` / `check-hub`) | Rust-ratio phone peer contract: `origin=apk_edge` `role=virtual_node` `class=edge` via `/api/edge` only. Never `:8091`. Telegram is proxy-only. Disk via `xtask::disk_report`. No gradle/Java in this crate. |
