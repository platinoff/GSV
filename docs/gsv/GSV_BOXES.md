...
| Tickets | `tickets/` (`boxes/tickets.rs`) | `GET`/`POST /api/tickets` · `POST /api/tickets/claim` · `/done` · `/error` · `/presence` · `/reclaim` · `/walk` · `/hook` · `GET`/`POST /api/tickets/bench` · `POST /api/tickets/next` | git JSONL board; scenario `tickets[]` bands; registered product; solo/squad; `lease_until` + stale reclaim; jail/`squad_cap` + join `env` (band **186**); MCP `gsv_tickets*` + `gsv_telegram_ticket` + `gsv_telegram_poll` + `gsv_telegram_decode` + `gsv_tickets_walk` + `gsv_tickets_hook` + `gsv_tickets_bench` + `gsv_tickets_next` (**55** tools); events in `ticket_claims.jsonl` — [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) **band 168 ✅ · 170 ✅ · 171 ✅ · 174 ✅ · 175 ✅ · 176 ✅ · 177 ✅ · 178 ✅ · 179 ✅ · 182 ✅ · 183** · [`GSV_SOLO_SQUAD_JAIL.md`](./GSV_SOLO_SQUAD_JAIL.md) **186** |
