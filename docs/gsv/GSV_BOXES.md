...
| **gsv_mcp_openbot** | `mcp.rs` + `gsv-mcp` bin | stdio + `GET`/`POST /mcp` + Galaxy card `/api/ui/card/mcp` | 26 box tools + 6 `gsv://` resources + 3 prompts + `logging/setLevel` + `completion/complete` + `resources/subscribe` + logging/resource notifications; клієнти: OpenCode / Cursor / Grok CLI / Grok Bot — [`GSV_MCP_OPENBOT.md`](./GSV_MCP_OPENBOT.md) |
