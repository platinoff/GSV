...
| OmniRouter | `omni/` (`boxes/omni/`) | `/api/omni/…` | shared catalog + `omni.toml` + quota timers; **band 225:** local provider `bunke-rock` / `lama-2.8` (kind `local`, gated on `models/Qwen3.8-27B-UD-IQ2_XXS.gguf` exists via `catalog::host_ready`); wire `ProviderWire.kind`; route task=rust picks it when prefer_free |
