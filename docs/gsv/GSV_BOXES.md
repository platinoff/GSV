...
| APK client | `apk/` (`boxes/apk.rs`) | `GET /api/apk` · bin `gsv-apk` (`disk` / `settings` / `adb` / `join` / `package` / `worker` / …) | Rust-ratio phone peer (`apk_edge`). Mini App/Chrome is not the worker (edge POST 403). Hub reads disk + settings JSON. WiFi ADB only. Never `:8091`. |
