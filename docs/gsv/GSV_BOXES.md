...
| APK client | `apk/` (`boxes/apk.rs`) | `GET /api/apk` · bin `gsv-apk` (`disk` / `settings` / `adb` / `join` / `package` / …) | Rust-ratio phone peer. Hub reads disk + settings JSON (no screenshot loops). WiFi ADB: pair/connect/logcat/`df` only. Never `:8091`. |
