...
| APK client | `apk/` (`boxes/apk.rs`) | bin `gsv-apk` (`disk` / `register --json` / `join --json` / `join --live` / `check-hub` / `telegram` / `service-account`) | Rust-ratio phone peer: `origin=apk_edge` via `/api/edge` + edge token (not poolAI admin). `join` POSTs health + register-remote + pool/join (dry-run default). Never `:8091`. Telegram is proxy-only. |
