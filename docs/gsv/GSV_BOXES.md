...
| Tickets | `tickets/` (`boxes/tickets.rs`) | `GET`/`POST /api/tickets` · `POST /api/tickets/claim` · `/done` · `/error` · `/presence` · `/reclaim` · `/walk` · `/hook` | git JSONL board; scenario `tickets[]` bands; registered product; solo/squad; `lease_until` + stale reclaim; MCP `gsv_tickets*` + `gsv_telegram_ticket` + `gsv_tickets_walk` + `gsv_tickets_hook` (**51** tools); events in `ticket_claims.jsonl` — [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) **band 168 ✅ · 170 ✅ · 171 ✅ · 174 ✅ · 175 ✅ · 176 ✅ · 177 ✅** |
