...
| Keep-live | `keep_live/` (`boxes/keep_live.rs`) | `/api/keep-live` · `/api/health { keep_live }` · MCP `gsv_keep_live` · Galaxy card `keep-live` | aggregation-only supervision of the always-on kit (**band 223**): `KeepLiveReport { gsv, telenetis, llama_rs, omniroute }` each `{ alive, url, version?, lag? }`, probes 1s, **fail-open** — `ok` stays true when a peer is down (like `disk_ok` band 181); no respawn; env overrides `GSV_KEEP_LIVE_GSV_URL` / `GSV_KEEP_LIVE_TELENETIS_URL` / `OMNIROUTE_URL` / `LLAMA_HEARTBEAT_PATH`; llama_rs alive = fresh `llama_heartbeat.json` (age ≤ 60s; llama-rs does not write it yet — band 225); `CARD_NAMES` **43** |
