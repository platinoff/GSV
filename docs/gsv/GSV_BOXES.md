...
| Telegram bus | same telegram box (**band 169 ✅** · **193** `kind:presence`) | MCP `gsv_telegram_bus_send` / `gsv_telegram_bus_poll` · `GET`/`POST /api/telegram/bus` | MCP bots talk via Godfather channel envelopes (dry-run in-memory queue; no webhook; no Cloudflare). Band **193**: federated jail heartbeat `kind:presence`. |
