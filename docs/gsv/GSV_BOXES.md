...
| Tickets | `tickets/` (`boxes/tickets.rs`) | `GET`/`POST /api/tickets` · `POST /api/tickets/claim` | git JSONL board (`docs/gsv/tickets.jsonl`); MCP `gsv_tickets` + `gsv_tickets_claim`; claim appends `ticket_claims.jsonl`; co-workflow `ticket-claim` gates claim — [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) **band 168 ✅** |
