...
| Telegram | `telegram/` (`boxes/telegram.rs`) | `GET /api/telegram` · `GET`/`POST /api/telegram/bus` · `POST /api/telegram/ticket` | Godfather bind (`getMe`+`getChat`); dry-run stub under cargo test / `X-Telegram-Dry-Run: 1`; poller default off; never `bot_token`; band **174** ticket ingest; band **175** `kind:sync` on solo walk; band **176** session lines + live `sendMessage` 1/s — [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) **band 167 ✅ · 174 ✅ · 175 ✅ · 176 ✅** |
| Tickets | `tickets/` (`boxes/tickets.rs`) | `GET`/`POST /api/tickets` · `POST /api/tickets/claim` · `/done` · `/error` · `/presence` · `/reclaim` · `/walk` · `/hook` · `GET`/`POST /api/tickets/bench` | git JSONL board; scenario `tickets[]` bands; registered product; solo/squad; `lease_until` + stale reclaim; MCP `gsv_tickets*` + `gsv_telegram_ticket` + `gsv_tickets_walk` + `gsv_tickets_hook` + `gsv_tickets_bench` (**52** tools); events in `ticket_claims.jsonl` — [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) **band 168 ✅ · 170 ✅ · 171 ✅ · 174 ✅ · 175 ✅ · 176 ✅ · 177 ✅ · 178 ✅** |
...
| Telegram walk sync | same telegram box (**band 176 ✅**) | MCP `gsv_tickets_walk` · `POST /api/tickets/walk` | session lines (`solo claimed` / `squad assigned` / `bench gsv_dev`); live send 1/s; dry-run queue |
...
| Scenario bench | tickets (**band 178 ✅**) | MCP `gsv_tickets_bench` · `GET`/`POST /api/tickets/bench` · `cargo xtask record-scenario-bench` | Instant `abrakadabra-session` create+walk; persist `docs/gsv/scenario_bench.json`; Godfather `session=` ns |
