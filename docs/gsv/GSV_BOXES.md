...
| Telegram | `telegram/` (`boxes/telegram.rs`) | `GET /api/telegram` · `GET`/`POST /api/telegram/bus` | Godfather bind (`getMe`+`getChat`); dry-run stub under cargo test / `X-Telegram-Dry-Run: 1`; poller default off; never `bot_token` — [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) **band 167 ✅** |
| Tickets | `tickets/` (`boxes/tickets.rs`) | `GET`/`POST /api/tickets` · `POST /api/tickets/claim` · `/done` · `/error` · `/presence` · `/reclaim` | git JSONL board; scenarios; registered product; solo/squad; `lease_until` + stale reclaim; MCP `gsv_tickets*` (47 tools); events in `ticket_claims.jsonl` — [`GSV_SETTINGS_TELEGRAM.md`](./GSV_SETTINGS_TELEGRAM.md) **band 168 ✅ · 170 ✅ · 171 ✅** |
