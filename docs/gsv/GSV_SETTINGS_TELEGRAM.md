...
4. Band **195** is landed: federated `kind:done` (the finisher posts the close; host board `in_progress` → `done`; ranks stay process-local). Band **194** is landed: federated `kind:claim` (remote jail claims an open row on this jail’s board; remote keeps its own `tickets.jsonl`). Band **193** is landed: federated `kind:presence`. Band **191** is landed: channel role host/mate/guest/local; GitHub origin probe (`github_ahead` even when local `src/` is not newer); ticket hook `github`; next-action picks the board then GitHub issues. Band **189** is landed: labeled Settings form, `squad_cap_override`, dark Galaxy scrollbars, MCP `catalog_stale` banner. Bands **166–195** are landed. Next drain: owner pick after a warnings-first scan.
