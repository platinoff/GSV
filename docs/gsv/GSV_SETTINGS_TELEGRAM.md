...
4. Band **187** is landed: live `getChatMemberCount` fills `tickets.member_count` (dry-run stub n=3 does not persist; poller ≥60s). Band **186** is landed: solo/squad/jail (`jail.id`; `squad_cap` = Godfather `member_count`; join `env`; presence cap; `gsv://docs/solo-squad-jail`). Band **185** is landed: Cursor catalog restart lockstep (`catalog_stale`; Galaxy **restart Cursor** — agent refresh does not re-list tools). Band **184** is landed: MCP session catalog lockstep (JSON `POST /mcp` keeps `notifications/tools/list_changed` for Cursor's GET hold). Band **183** is landed: squad next-action (`gsv_tickets_next`) + MCP `notifications/tools/list_changed`. Bands **166–187** are landed. Next drain: owner pick after a warnings-first scan.
