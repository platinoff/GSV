...
**Status:** Bands **166–189** (Telegram Godfather through Settings Galaxy polish). Latest: **189** labeled Settings form + `squad_cap_override` + dark scrollbars; MCP `catalog_stale` banner. **187** `getChatMemberCount` fills `tickets.member_count` / derived `squad_cap`. **186** jail identity + `squad_cap` = channel members + join `env`. **185** `catalog_stale` / Galaxy **restart Cursor** (agent refresh does not re-list). **184** JSON POST keeps `tools/list_changed` for the SSE hold · `listed_tool_count`. **183** `gsv_tickets_next` + MCP `tools/list_changed`. **182** dual session line + JSON `data` / Galaxy MCP signal / `gsv_telegram_decode`. **180–181** are watchdog/health (roadmap). **Next drain:** owner pick after a warnings-first scan.  
**Date:** 2026-08-20  
**Deciders:** owner  
...
4. Band **189** is landed: labeled Settings form, `squad_cap_override`, dark Galaxy scrollbars, MCP `catalog_stale` banner. Band **187** is landed: live `getChatMemberCount` fills `tickets.member_count` (dry-run stub n=3 does not persist; poller ≥60s). Band **186** is landed: solo/squad/jail (`jail.id`; `squad_cap` = Godfather `member_count`; join `env`; presence cap; `gsv://docs/solo-squad-jail`). Band **185** is landed: Cursor catalog restart lockstep (`catalog_stale`; Galaxy **restart Cursor** — agent refresh does not re-list tools). Band **184** is landed: MCP session catalog lockstep (JSON `POST /mcp` keeps `notifications/tools/list_changed` for Cursor's GET hold). Band **183** is landed: squad next-action (`gsv_tickets_next`) + MCP `notifications/tools/list_changed`. Bands **166–189** are landed. Next drain: owner pick after a warnings-first scan.
