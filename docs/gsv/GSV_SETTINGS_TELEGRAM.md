...
**Status:** Bands **166–196** (Telegram Godfather through federated `kind:reclaim`). Latest: **196** lease expiry or an explicit release posts `kind:reclaim`; peer boards transition their copy `in_progress` → `open` (+ `kind:reclaimed`); ranks stay process-local; guest mute; echo skip. **195** the jail that finished a claimed row posts `kind:done`; host board transitions it `in_progress` → `done`. **194** remote jail claims an open host-board ticket via Godfather; guest mute; echo skip; local `try_dispatch` stays process-local. **193** federated `kind:presence`. **191** channel roles host/mate/guest/local + GitHub origin (`github_ahead` when the installer has no newer local tree) + ticket pick board-then-GitHub. **189** labeled Settings form + `squad_cap_override` + dark scrollbars; MCP `catalog_stale` banner. **187** `getChatMemberCount` fills `tickets.member_count` / derived `squad_cap`. **186** jail identity + `squad_cap` = channel members + join `env`. **185** `catalog_stale` / Galaxy **restart Cursor** (agent refresh does not re-list). **184** JSON POST keeps `tools/list_changed` for the SSE hold · `listed_tool_count`. **183** `gsv_tickets_next` + MCP `tools/list_changed`. **182** dual session line + JSON `data` / Galaxy MCP signal / `gsv_telegram_decode`. **180–181** are watchdog/health (roadmap). **Next drain:** owner pick after a warnings-first scan.  
**Date:** 2026-08-20  
**Deciders:** owner  
...
4. Band **196** is landed: federated `kind:reclaim` (lease sweep or explicit release posts the reclaim; peer boards go back to `open`; ranks stay process-local). Band **195** is landed: federated `kind:done` (the finisher posts the close; host board `in_progress` → `done`; ranks stay process-local). Band **194** is landed: federated `kind:claim` (remote jail claims an open row on this jail’s board; remote keeps its own `tickets.jsonl`). Band **193** is landed: federated `kind:presence`. Band **191** is landed: channel role host/mate/guest/local; GitHub origin probe (`github_ahead` even when local `src/` is not newer); ticket hook `github`; next-action picks the board then GitHub issues. Band **189** is landed: labeled Settings form, `squad_cap_override`, dark Galaxy scrollbars, MCP `catalog_stale` banner. Bands **166–196** are landed — the federation lifecycle (presence → claim → done → reclaim) is complete. Next drain: owner pick after a warnings-first scan.
...
### P2 — Should (band 187) — live Godfather member_count

Owner pick: band 186 left `member_count` owner-set. Live channel was bound but count stayed 0 → `squad_cap` = 50 bot slots.

| Piece | Acceptance |
|-------|------------|
| Probe | `getChatMemberCount` + `getChat.type` on live `GET /api/telegram` / `gsv_telegram`. |
| Stub | Dry-run `member_count` 3; does **not** write settings. |
| Persist | Live count >0 writes `tickets.member_count`; `squad_cap` follows unless owner override. Failed probe does not wipe. |
| Poller | Live `poll_once` refreshes ≥60s. |
| Galaxy | Telegram card members row. `CARD_NAMES` **40**. |
| Hygiene | `docs/gsv/ticket_claims.jsonl` gitignored (runtime). |
...
- Band 194: `kind:claim` federated ticket claim on the host board; guest mute; echo skip; remote keeps its own `tickets.jsonl`; `--stretch-96` ≥ 96%.
- Band 195: `kind:done` federated ticket close on the host board (`in_progress` → `done` via `tickets::done_remote`); ranks stay process-local; guest mute; echo skip; `--stretch-96` ≥ 96%.
- Band 196: `kind:reclaim` federated release on the host board (lease sweep + explicit wire → `tickets::reclaim_remote`, `in_progress` → `open` + `kind:reclaimed`); ranks stay process-local; guest mute; echo skip; `--stretch-96` ≥ 96%.
...
| **194** | S2579–S2588 | Federated `kind:claim` on host board | **✅ this drain** |
| **195** | S2589–S2598 | Federated `kind:done` on host board | **✅ this drain** |
| **196** | S2599–S2608 | Federated `kind:reclaim` on host board | **✅ this drain** |
