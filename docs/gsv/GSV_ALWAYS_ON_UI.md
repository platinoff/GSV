...
Always-on horizon **closed**. MCP catch-up **band 151–152 ✅**. Rust-first xtask **band 153 ✅**. Watchdog ops card + fingerprint model **band 154 ✅**. Session token usage **band 155 ✅**. Streaming usage + VDT git + owner tunnel **band 156 ✅**. Omni catalog **band 157 ✅**. Live MCP stdio + sync check **band 158 ✅**. Cursor HTTP MCP + session SSE hold **band 159 ✅**. GSV sandbox MCP / no User leak **band 160 ✅**. Vision lockstep + disk MiB / `--clean` keep-live **band 161 ✅**. Live crate/version lockstep **band 162 ✅**. Vision queue lockstep + bump auto-advance **band 163 ✅**. Cursor 3.16.29 kit lockstep **band 164 ✅**. Next `абракадабра` on **gsv**: scan first, then owner pick.
