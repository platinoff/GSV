...
**Status:** Bands **186–195**. **195** federated `kind:done`. **194** federated `kind:claim`. **193** federated `kind:presence`. **192** ranks + no CMD flash. **191** host/mate/guest/local + GitHub origin lockstep. **187** live `getChatMemberCount`. Bands **166–185 ✅**.  
**Date:** 2026-08-20  
**Deciders:** owner  
...
Catalog ids: `federated-join` · `own-channel` · `jail-app` · `federated-presence` · `federated-claim` · `federated-done` (plus existing `squad-dev` / `telegram-solo` / `abrakadabra-session` / `rank-ladder`).
...
- Scenarios `federated-join` / `own-channel` / `jail-app` exist.
