...
**Оновлено:** 2026-08-19 (band 173 ✅ · next = **owner pick**)
...
**band 167 ✅** Telegram bind · **band 168 ✅** ticket board + MCP claim · **band 170 ✅** ticket scenarios + solo/squad MCP · **band 171 ✅** ticket lease + stale reclaim in the same spec. **band 172 ✅** live crate lockstep. **band 173 ✅** vision queue close-lockstep. MCP canon: [`gsv/GSV_MCP_OPENBOT.md`](gsv/GSV_MCP_OPENBOT.md).
...
- **GSV** — окремий Rust-first проєкт (`S:\rust\GSV`), bands 102 · 108–121 · 125–173 **✅**.
...
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.35%** (rust 30559 / product 30759) · **577** tests · clippy 0 · fmt clean.
- **Сервер:** canon порт **9999** (`DEFAULT_PORT`; 8765 — Hyper-V reserved range).
- **Vision rev:** **516** (band 173 `cargo xtask sync`; last `PH-S2378` · next `PH-S2379`).
