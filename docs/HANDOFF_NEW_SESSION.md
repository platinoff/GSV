...
**Оновлено:** 2026-08-19 (band 174 ✅ · next = **owner pick**)
...
**band 167 ✅** Telegram bind · **band 168 ✅** ticket board + MCP claim · **band 170 ✅** ticket scenarios + solo/squad MCP · **band 171 ✅** ticket lease + stale reclaim in the same spec. **band 172 ✅** live crate lockstep. **band 173 ✅** vision queue close-lockstep. **band 174 ✅** solo Telegram tickets. MCP canon: [`gsv/GSV_MCP_OPENBOT.md`](gsv/GSV_MCP_OPENBOT.md).
...
- **GSV** — окремий Rust-first проєкт (`S:\rust\GSV`), bands 102 · 108–121 · 125–174 **✅**.
- **Next drain (gsv):** **owner pick** after warnings-first scan. `cargo xtask bump --band N` locksteps last/next/active to the **close** of N.
- **Band 174:** Solo bot tickets from Telegram — `/ticket` or `{kind:ticket}` → board row; solo MCP auto-claims; MCP `gsv_telegram_ticket` (**48** tools); `CARD_NAMES` **40**.
...
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.36%** (rust 31107 / product 31307) · **584** tests · clippy 0 · fmt clean.
- **Сервер:** canon порт **9999** (`DEFAULT_PORT`; 8765 — Hyper-V reserved range).
- **Vision rev:** **516** (band 174 `cargo xtask sync`; last `PH-S2388` · next `PH-S2389`).
