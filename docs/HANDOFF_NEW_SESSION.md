...
**Оновлено:** 2026-08-24 (band **203** · next = **owner pick**)
...
+ **band 204** logic-audit VIII src sweep complete (one confirmed fix: `SseUsageTap` byte-boundary chunk drop)
...
- **GSV** — окремий Rust-first проєкт (`S:\rust\GSV`), bands 102 · 108–121 · 125–204.
- **Band 204:** Logic-audit VIII (owner pick) — src sweep complete: every remaining surface audited across bands 197–204 (~25 surfaces). One confirmed bug: `usage::SseUsageTap::push` dropped an **entire chunk** when a multibyte UTF-8 char straddled the chunk boundary (`from_utf8(chunk)` fail → early return) — complete `data:` usage lines inside that chunk were lost, undercounting streamed tokens; tap now buffers raw **bytes** (`buf: Vec<u8>`), splits on `b'\n'`, decodes per line via `from_utf8_lossy`; `flush()` checks all-ascii-whitespace instead of `trim().is_empty()`. Regression: `sse_tap_keeps_usage_when_chunk_splits_multibyte_char` (orphan `0xC3`/`0xA9` split + usage in the second segment — fails on the old code). Cosmetic notes only elsewhere: `preview::highlight_rs` over-consumes after a string ending in an even backslash run (display-only, escaped); `omni/quota.rs` unknown task falls back to rust∩web rather than any; `omni/config.rs` apply_patch is partial-on-error; `github.rs parse_triple` treats prerelease as `.0`; `schtasks /TR` breaks if repo path has spaces. Gate: **699** tests · clippy 0 · fmt clean.
