...
## 9. Згода на тести з хоста (галочка в аплікації)

**Вердикт: без галочки хост не тестить; з галочкою — тільки `test_ping`.**

- ✅ Галочка в Mini App + POST `/api/testmode` (initData; identity з перевіреного
  `user.id`, не з тіла). Вимкнення видаляє запис — відсутність = нема згоди.
- ✅ Харнес читає `GET /api/edge/testmode/peers` (initData або прямий LAN) і
  ставить задачі тільки тим, хто в списку. Воркер виконує `test_ping`
  (pong без моделі) тільки з увімкненим локальним тоглом, інакше requeue.
- ✅ v1 allowlist: `test_ping` і крапка — ніяких автоскачувань, ніяких моделей.
- 🚫 Обхід згоди (харнес в обхід реєстру, задачі поза allowlist) — не робимо.
...
19. ✅ T16 done 2026-09-17: згода на тести (реєстр + toggle + `test_ping`, емулятор-луп); харнес тільки по списку.
