...
| test_ping з'їдено (`llama_edge: ... ignored` у результаті) | Чергу пира ганяють PC-полери `llama_edge.exe` (llama-rs) — вони закривають чужі типи як ignored. Спочатку впевнись що воркер на телефоні **Started** (без нього телефон взагалі не бере участі); якщо стартував а їдять далі — треба фікс `llama_edge` (requeue замість complete), це інше репо, рішення власника |
