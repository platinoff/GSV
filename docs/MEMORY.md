...
## Стан (2026-09-09 · band 229)

- **Band 229:** rules-drift check (Gravity harness) — `cargo xtask rules-check` = aggregated rules-vs-live drift report. `src/boxes/rules.rs` (new box): `collect(repo_root, data_dir, live_url) -> RulesCheck { ok, at, git_head, doc, checks[] }` (rows `RuleCheck { id, ok, gate, detail }`). Hard gates: `git` (HEAD resolvable), `fingerprint` (crate version recorded for product `gsv`; HEAD-match advisory — fingerprint writes pre-commit), `vision` (`collect_drift` clean), `ratio` (≥ 95% via `data/rust_ratio.json`, 96% stretch advisory). Soft (never flip exit): `registry` (rows keep HANDOFF+NEXT via `products::scan`), `sli` (used vs unused new-SLI pool), `bench` (`speed_index_fresh` ≤ 31 d — parses RFC3339 **and** date-only `2026-09-09`), `live` (`keep_live::probe_http_blocking` vs `gsv_version()`, `:9999` /api/watchdog version == crate). CLI `rules-check` (`--no-live`; `GSV_LIVE_URL`) + MCP read-only task `rules` (MCP_TASKS = 5). Doc `docs/gsv/GSV_RULES_CHECK.md`; MCP resource `gsv://docs/rules-check` (doc-URI completion **11**); BOXES row; RUST_DEV/MCP_OPENBOT/SERVER/AGENTS surfaces. Contracts: 5 rules unit tests → lib **315**; xtask contracts assert 5-task catalog + `rules` wire; watchdog contracts now **39** (tautological probe deleted). Post-bump in-tree: `rules-check ok` exit 0 (bench fresh, live 0.229.0 == crate, vision rev 516, ratio 99.48%). Gate: fmt clean · clippy 0 · full `cargo test` green (42 suites) · stretch-96 **99.48%** (45106/45343) · record-rust 0. Version **0.229.0** (`cargo xtask bump --band 229`; queue last `PH-S2938` next `PH-S2939`).
- **Canon:** [`gsv/GSV_TECH_ROADMAP.md`](gsv/GSV_TECH_ROADMAP.md) (band 229 table, PH-S2888…S2894 ✅).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → 99.48% (post-bump) · full `cargo test` green · clippy 0
...
- **Band 223:** Keep-live health aggregation (owner pick) — GSV supervises the always-on kit (GSV :9999 + Telenetis :9800 + llama-rs + OmniRoute) without respawn. `src/boxes/keep_live.rs`: `KeepLiveReport { gsv, telenetis, llama_rs, omniroute }` each `{ alive, url, version?, lag? }`, probes 1s, **fail-open** (`ok` stays true when a peer is down — same principle as `disk_ok` band 181); env overrides `GSV_KEEP_LIVE_GSV_URL` / `GSV_KEEP_LIVE_TELENETIS_URL` / `OMNIROUTE_URL` / `LLAMA_HEARTBEAT_PATH`; llama_rs alive = fresh `llama_heartbeat.json` age ≤ 60s (file probe, no HTTP; llama-rs writes it in band 225 ✅). Wire: `GET /api/keep-live` + `GET /api/health { keep_live }` via async `wire_async`; MCP `gsv_keep_live` read-only → **57** tools · **13** `gsv://` resources; Galaxy studio card `keep-live` (`CARD_NAMES` **43**, 4-peer table peer/alive/url/version); `src/bin/gsv_http_stand_smoke.rs` CARDS **43**. Contracts `tests/gsv_keep_live_contracts.rs`: dead ports → `ok:true` + `alive:false`, render rows/empty/error, card list. Fixed **pre-existing recursive `cargo test` hang**: `terminal_cargo_test_allowed` ran a nested `cargo test` (build-lock deadlock + infinite recursion) → `terminal_cargo_version_allowed` (`cargo --version`) + whitelist unit assert (`validate("cargo test").is_ok()`). Misc: `docs/gsv/GSV_BOXES.md` keep-live row, `GSV_MCP_OPENBOT.md` Tools **57** + status band 223, roadmap rows band 223 ✅. Crash: none. Vision rev **516**. Queue: last `PH-S2878` next `PH-S2879` (band 224).
- **Canon:** [`gsv/GSV_TECH_ROADMAP.md`](gsv/GSV_TECH_ROADMAP.md) (band 223 table, PH-S2852…S2859; band 225 PH-S2867…S2874; code lockstep PH-S2869…S2878).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.46%** (rust 43541 / product 43778) · **782** tests · clippy 0

## Стан (2026-08-24 · band 207)

- **Band 207:** Logic-audit XI (owner pick) — MCP protocol surface end-to-end: dispatch (`src/mcp.rs`), `/mcp` HTTP handlers (`src/server/mod.rs`), stdio bin. Five confirmed fixes: (1) re-initialize carrying a live `Mcp-Session-Id` minted a duplicate session and handed back the new id while the old stayed live until cap eviction — now reuses the validated header id; (2) an `[initialize, tools/list]` batch marked listed-tools on the header session (`None` for fresh clients) so the issued id never got its `listed_tools` — `handle_value_in` receives the effective (issued/reused) id; (3) notifications-only JSON POST answered 204 No Content where MCP Streamable HTTP requires 202 Accepted — both JSON and empty-SSE-notes branches return 202; (4) `tracker_payload` took two separate `try_read` acquisitions, so a tracker push between them mixed sprints/records generations — single lock acquisition; (5) `completion/complete` truncated matches at `COMPLETION_MAX` then reported `total = page length`, `hasMore: false` — extracted `complete_page(pool, prefix, max)` returning honest `(values, total, has_more)`. Mechanical check: TOOL_NAMES ↔ `call_tool` arms parity 56↔56. Regressions: `reinitialize_reuses_live_session`, `initialize_batch_marks_listed_on_issued_session`, `notification_only_post_returns_accepted`, `notification_only_batch_returns_accepted`, `complete_page_truncates_honestly`.
- **Canon:** [`gsv/GSV_TECH_ROADMAP.md`](gsv/GSV_TECH_ROADMAP.md) (band 207 table).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.44%** (rust 41822 / product 42059) · **712** tests · clippy 0

## Стан (2026-08-24 · band 206)

- **Band 206:** Logic-audit X (owner pick) — first sweep of the two surfaces no prior audit had read end-to-end: Galaxy JS glue (`ui/index.html`, 239 LOC) and the bench harness (`benches/gsv_dev.rs`). Three confirmed fixes: (1) `resync` computed `failed` from rejected promises only, but every awaited wrapper (`refreshMeta`/`refreshTrail`/`loadRssTicker`/`getText`) catches internally — `failed` was always 0, so each silent resync flipped the badge back «online» even with the server down (contradicts offline-stable band 125 and live-swap band 144); `refreshMeta`/`getText` now resolve `true` on success / `false` on failure and `failed` counts `rejected || value===false`; the 30s interval's `.catch(()=>setOffline(true))` was dead for the same reason — plain `setInterval(refreshMeta,30000)`. (2) `hookTickets` fell back to `{source:"band",id:"177",from:"solo"}` on a bare click — silently spawning duplicate tickets of a closed band; now toasts «hook needs a scenario id, band id, plan stem, or phrase» without posting. (3) bench `"disk_report"` arm was unreachable (name absent from the run list) and `_ => {}` let any future typo benchmark an empty loop and print fake numbers — arm wired into the list, unknown name is now `unreachable!`, and the `median-ish` output label is honestly `mean`. Polish: `cycleGpuMode` swaps via exact `classList.remove/add` instead of regex-rebuilding body className; Auto toggle's dead `.off` class styled `.badge.off{opacity:.55}` so Manual state is visible. Contracts: `ui_index_resync_offline_follows_results`, `ui_index_hook_requires_input` (served-HTML shape style per house precedent). Env note: this box's MSYS2 bash mangles `${VAR:-default}` expansions through the PowerShell hop — hardcode `/c/Users/plati/.cargo/bin` in PATH and skip `RUSTUP_TOOLCHAIN` (gnu toolchain already default).
- **Canon:** [`gsv/GSV_TECH_ROADMAP.md`](gsv/GSV_TECH_ROADMAP.md) (band 206 table).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.43%** (rust 41668 / product 41905) · **708** tests · clippy 0

## Стан (2026-08-24 · band 205)

- **Band 205:** Logic-audit IX (owner pick) — closed every leftover cosmetic note from bands 197–204. Five confirmed fixes: (1) `omni/quota.rs pick_route` routed an unknown task (`code`, typos, `x-omni-task` garbage) as rust∩web — both `want_rust`/`want_web` were false so the filter collapsed to `m.rust && m.web`; extracted `task_matches` (rust/web exact lanes, everything else = union like "any"); (2) `omni/config.rs apply` mutated the live config in place — a late patch error (unknown provider, priority overflow) left it half-applied while `persist()` was skipped (memory ≠ disk until restart); patch now stages on a clone and `mem::swap`s only on success; (3) `github.rs parse_triple("0.205.0-rc.1")` silently parsed "0-rc" as 0 — numeric core strips `-pre/+build`; equal triples tie-break release > prerelease in `version_gt`; (4) `preview::highlight_rs` closed string literals with `ends_with("\\\"")`, so a quote after an even backslash run swallowed the rest of the line into the span — scanner counts the run (even closes, odd-run escaped quote resets); (5) `xtask install_watchdog` glued unquoted paths into the persisted `TR` — repo roots with spaces split at the space; `watchdog_task_tr` quotes exe + root for `schtasks /TR` and HKCU Run. Regressions: `task_matches_unknown_task_is_union_not_intersection`, `pick_route_unknown_task_equals_any`, `apply_patch_error_leaves_config_untouched`, `version_gt_prerelease_ranks_below_release`, `highlight_even_backslash_run_closes_string`, `highlight_odd_backslash_run_keeps_quote_escaped`, `watchdog_task_tr_quotes_paths_with_spaces`. S0 note: this box has no rustup — MSYS2 `rustc/cargo`; exporting `RUSTUP_TOOLCHAIN` per AGENTS.md breaks every cargo call ("override toolchain not installed") — `unset RUSTUP_TOOLCHAIN` first.
- **Canon:** [`gsv/GSV_TECH_ROADMAP.md`](gsv/GSV_TECH_ROADMAP.md) (band 205 table).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.43%** (rust 41625 / product 41862) · **706** tests · clippy 0

## Стан (2026-08-24 · band 204)

- **Band 204:** Logic-audit VIII (owner pick). Completed the src sweep — every remaining surface now audited at least once across bands 197–204: `server/mod.rs`, `omni/{catalog,config,quota,proxy,mod}.rs`, `security.rs`, `state.rs`, `watchdog.rs`, `github.rs`, `xtask.rs`, `terminal.rs`, plus the remainders of `usage`, `ratio`, `tracker`, `gitkit`, `sw`, `mds`, `sli`, `toolchain`, `guide`, `hooks`, `fingerprint`, `products`, `preview` (~25 surfaces, ~9k LOC). One confirmed bug: `usage::SseUsageTap::push` dropped an **entire chunk** when a multibyte UTF-8 char straddled the chunk boundary (`from_utf8(chunk)` fails → early return) — any complete `data:` usage line inside that chunk was lost, undercounting streamed tokens; tap now buffers raw bytes and decodes per complete line (`from_utf8_lossy`), `flush` same treatment. Regression: `sse_tap_keeps_usage_when_chunk_splits_multibyte_char` (orphan `0xC3`/`0xA9` split + usage in the second segment — fails on the old code). Cosmetic notes only elsewhere: `preview::highlight_rs` over-consumes after a string ending in an even backslash run (display-only, escaped); `omni/quota.rs` unknown task falls back to rust∩web rather than any; `omni/config.rs` apply_patch is partial-on-error; `github.rs parse_triple` treats prerelease as `.0`; `schtasks /TR` breaks if repo path has spaces. Gate: **699** tests · clippy 0 · fmt clean.
- **Canon:** [`gsv/GSV_TECH_ROADMAP.md`](gsv/GSV_TECH_ROADMAP.md) (band 204 table).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → measured at close · **699** tests · clippy 0

## Стан (2026-08-24 · band 203)

- **Band 203:** Logic-audit VII — tests sweep (owner pick: hunt vacuous / tautological assertions across all of `tests/*.rs`). Verdict: **clean** — zero fixes required; suite contracts confirmed meaningful. Method: mechanical greps first (self-comparison asserts, assertion-less test fns, always-true shapes, `.contains("")` — all zero), then full end-to-end reads of the twelve smaller files, then assert-line inventories for the nine large ones (1 695 assert lines). The seven `\|\| is_null/is_none` OR-shape sites verified as legitimate optional-field wire contracts (`tickets` claimed_by/lease_until absent-or-null, `server` sprints.total/selection optional-typed, `watchdog` crate_version string-or-null, `telegram` unclaimed claimed_by null, `products` selected null before select) — each CAN fail if the field appears with a wrong type, so none are tautological. Known-intentional non-defect: `gsv_stand_smoke_contracts::smoke_report_shape_is_stable` builds a JSON literal and asserts its own shape — wire documentation, since bin internals are not importable from integration tests. Gate unchanged: 698 tests, clippy 0, fmt clean; docs-only close. S0 note: pointing the Grep tool at a single FILE path searches the whole parent directory instead — extract per-file assert inventories via bash redirects (`grep -n assert <file> > tmp/inv.txt`).
- **Canon:** [`gsv/GSV_TECH_ROADMAP.md`](gsv/GSV_TECH_ROADMAP.md) (band 203 table).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.43%** (rust 41474 / product 41711) · **698** tests · clippy 0

## Стан (2026-08-24 · band 202)

- **Band 202:** Logic-audit fixes VI (owner pick: last unaudited surfaces — `mcp.rs` full dispatch, `boxes/ui.rs` renderers, `app_error.rs`, `lib.rs`, `vision.rs`, all eleven `src/bin/*`). Two confirmed bugs: (1) `ui::esc` escaped only `& < >` but every renderer interpolates its output into single-quoted HTML attributes (`title='…'` rss-ticker, `value='…'` settings inputs) — any wire string containing `'` broke out of the attribute and could inject extra attributes; quote entities (`&#39;` / `&quot;`) folded into `esc()` itself so all renderers are covered at once and guide's `esc_attr` output stays byte-identical; (2) vacuous `vision.rs` test asserted `git_head(&tmp).is_none() || git_head(&tmp).is_some()` — can never fail; rewritten as a real contract (in-repo Some non-empty hex ≤40 chars; outside None-or-valid). Everything else traced clean; cosmetic notes: `gsv_server --port abc` silently falls back to DEFAULT_PORT; `cargo xtask vault-note --title help` prints help (`any(== "help")`); MCP `gsv_update` reads a `check` arg missing from its declared inputSchema. Regressions: `esc_neutralizes_single_quote_attribute_breakout`, `git_head_short_hash_in_repo_and_none_or_short_outside`. S0 note: PowerShell strips double quotes inside `bash -lc '…'` strings — grep patterns with quotes/parens silently mangle; use file-based Read/Grep tools or redirect to a temp log.
- **Canon:** [`gsv/GSV_BOXES.md`](gsv/GSV_BOXES.md) · [`gsv/GSV_MCP_OPENBOT.md`](gsv/GSV_MCP_OPENBOT.md).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.43%** (rust 41474 / product 41711) · **698** tests · clippy 0
...
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.43%** (rust 41376 / product 41613) · **695** tests · clippy 0

## Стан (2026-08-22 · band 199)

- **Band 199:** Logic-audit fixes III (owner pick: finish the sweep). Covered the files no band had opened: fingerprint / xtask / omni router core / server / vision helpers / ui render spot-checks / mcp dispatch. Three confirmed fixes: (1) `omni::OmniRouter::persist` / `persist_quota` used `try_read().unwrap_or_default()` — under lock contention they serialized a **default** config/quota over tuned `data/omni.toml` and `data/omni_quota.json` (wiping provider overrides + cooldown timers); both now skip when contended; (2) `fingerprint::read_tail_text` seeked to `len-64KiB` and `read_to_string` — a multibyte char split at the boundary failed the whole read, so model discovery silently returned `unknown`; drops the partial prefix now; (3) `fingerprint::parse_cargo_version` matched any `version*` key (`version.workspace = true` → garbage version); only plain `version = "X"` keys count now, `bump_package_version` tightened the same way. Regressions: 3 omni persist tests + `fingerprint_log_tail_survives_multibyte_boundary` (exact-boundary 'é' fixture) + `fingerprint_pkg_version_ignores_version_workspace_and_prefixed_keys`. S0 note: disk hit **0 MiB free** during verify; `cargo xtask disk --clean` removed debug caches (live kept), 24 GiB free.
- **Canon:** [`gsv/GSV_OMNI_CATALOG.md`](gsv/GSV_OMNI_CATALOG.md).
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.43%** (rust 41235 / product 41472) · **692** tests · clippy 0
...
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.42%** (rust 35469 / product 35676) · **633** tests · clippy 0
...
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** measured at band close (`gsv-loc-audit --stretch-96`).
...
- **Next drain:** **owner pick** after a warnings-first scan.
- **VDT kit:** `абракадабра` / `abrakadabra` Step 0 is `cargo xtask products`.
- **Ratio / тести:** `gsv-loc-audit --stretch-96` → **99.40%** (rust 33352 / product 33555) · **607** tests · clippy 0
