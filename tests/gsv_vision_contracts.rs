...
#[test]
fn vision_queue_lockstep_ph_s2269() {
    let m = vision::read_manifest(&repo_root()).expect("manifest");
    let e = vision::read_extensions(&repo_root()).expect("extensions");
    assert_eq!(m.next_sprint, "PH-S2269", "manifest next_sprint");
    assert_eq!(e.active_sprint, "PH-S2269", "extensions active_sprint");
    assert_eq!(m.last_sprint_closed, "PH-S2268", "last closed is band 162");
}
