...
- **Resources:** `gsv://vision/{manifest,feed,extensions}` + `gsv://docs/{mcp-openbot,handoff,next,fingerprints,post-always-on,rust-dev,omni-catalog,settings-telegram,solo-squad-jail,ranks,rules-check,vdc,agi-path}` (allowlist; `..` / `file://` → `-32602`). `gsv_vision_sync` notifies every subscribed `gsv://` URI.
...
- **Rust folder is the host.** GSV kit + `gsv-server` live in that tree. Every other git tree is a **portable plugin** (own repo, own `target/`, own rules). Discovery does not copy the kit and does not widen MCP sandbox. Canon: [`docs/gsv/GSV_AGI_PATH.md`](docs/gsv/GSV_AGI_PATH.md) (`gsv://docs/agi-path`). Session law: `agi`.
- Global scope is the GSV kit only: MCP toolchains, environment flows (terminal, `rustup`, env), IDE/Cursor/OpenCode/Grok lockstep, model life & communications with connected release apps (`gsv-server` live). Plugin code and its local rules stay in the plugin repo — GSV never absorbs them.
- **Environment security first.** Folder MCP only (never User-scope). Preview/terminal/vision/xtask stay inside this crate. No tunnel, no `products/open` on MCP. No `:8091` off-box. Never echo tokens. Work stays inside the Rust environment (the repo tree + `target/`). No roaming to Windows temp/tmp (`C:\...\temp`, `C:\tmp`, `/tmp`, `%TEMP%`, `%TMP%`) for scratch files, downloads, or intermediate tooling. Anything extra needed (docs, references, drafts) lives inside the open repo or under `target/live/` — never outside the workspace.
