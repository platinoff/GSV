...
pub const TASKS: &[(&str, &str)] = &[
    ...
    ("disk", "S0 disk guard (free GiB + target/ size)"),
    (
        "live",
...
pub fn disk_report(repo_root: &Path, enforce: bool) -> DiskReport {
    ...
    let free_gb = volume_free_bytes(repo_root).map(|b| b / 1024 / 1024 / 1024);
    let target_gb = if target_dir.is_dir() {
        dir_size_bytes(&target_dir) / 1024 / 1024 / 1024
    } else {
        0
    };
    let mut violation = false;
    if let Some(free) = free_gb {
        if min_free_gb > 0 && free < min_free_gb {
            notes.push(format!(
                "free disk ({free} GiB) < GSV_MIN_FREE_DISK_GB ({min_free_gb})"
            ));
            violation = true;
        }
    } else {
    ...
    if max_target_gb > 0 && target_gb > max_target_gb {
        notes.push(format!(
            "target dir (~{target_gb} GiB) > GSV_MAX_TARGET_DIR_GB ({max_target_gb})"
        ));
        violation = true;
    }
    let ok = !(enforce && violation);
    DiskReport {
        ok,
        repo: products::display_path(repo_root),
        target_dir: products::display_path(&target_dir),
