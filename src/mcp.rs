...
pub fn tools_list() -> Vec<Value> {
    vec![
        ...
        tool("gsv_products", "VDT environment: rust-folder HOST + portable plugins (workspace ∪ sibling git ∪ kit). role=host|plugin, target_dir per tree.", object_schema()),
        tool(
            "gsv_products_scan",
            "Scan one discovered product (git HEAD, HANDOFF/NEXT, role host|plugin, target_dir). id optional after gsv_products_select.",
            json!({
                "type": "object",
                "properties": {
        ...
        tool(
            ...
            "Telenetis (:9800) keep-live health — alive, version, latency, uptime (fail-open).",
            object_schema(),
        ),
        ...
        tool(
            "gsv_ranks",
            "Merit ranks (IT + army mix). Jun-nub is L0; marshal-orchestrator is L15 and the channel-host display. Ranks are earned only: +1 on ticket done, -1 on ticket error, both to the claimer. Direct award/demote were removed (self-asserted identity). review with tests_ok=false demotes the fingerprint owner behind git_head once. Telegram ids redacted to a tail. Store is data/gsv_ranks.json (gitignored).",
            json!({
                "type": "object",
                "properties": {
                    "action": { "type": "string", "description": "list (default) | review" },
                    "actor": { "type": "string" },
                    "ide": { "type": "string" },
                    "agent": { "type": "string" },
        ...
        tool(
            ...
            json!({
                "type": "object",
                "properties": {
                    "scenario_id": { "type": "string", "description": "Named scenario (abrakadabra-session / agi-session / memory-disk-speed). Creates the band when set." },
...
const RESOURCES: &[ResourceSpec] = &[
    ...
    ResourceSpec {
        uri: "gsv://docs/agi-path",
        name: "AGI path / super MCP hub",
        description: "Rust-folder host, portable plugins, env security first, max Rust, agi session. Super MCP hub operator setup.",
        mime: "text/markdown",
        rel: "docs/gsv/GSV_AGI_PATH.md",
    },
];
...
pub fn http_info(state: &AppState) -> Value {
    ...
    json!({
        ...
        "catalog_notify": true,
        "keep_live_summary": state.keep_live_cached().unwrap_or_else(|| json!({})),
        "tools": tools,
...
async fn call_tool(state: &AppState, params: &Value, session: Option<&str>) -> Value {
    ...
    match name {
        ...
        "gsv_telenetis_health" => tool_ok(crate::boxes::keep_live::telenetis_wire_async().await),
        ...
        "gsv_telegram_bus_send" => {
            let dry = crate::boxes::telegram::env_dry_run();
            let v = crate::boxes::telegram::bus_send_checked(
                &state.data_dir,
                dry,
                &args,
...
mod tests {
    ...
    async fn resources_list_is_allowlisted_gsv_uris() {
        ...
        assert!(RESOURCE_URIS.contains(&"gsv://docs/agi-path"));
    ...
    async fn drain_prompt_names_always_on_tools() {
        ...
        assert!(text.contains("trigger agi"), "{text}");
        ...
        assert!(
            text.contains("from must be your presence identity"),
            "{text}"
        );
        assert!(text.contains("leases auto-reclaim"), "{text}");
        assert!(text.contains("earned only"), "{text}");
    }
    ...
    async fn products_select_then_scan_omits_id() {
        ...
        assert!(
            text.contains("\"role\":\"host\"") || text.contains("\"role\": \"host\""),
            "{text}"
        );
        assert!(text.contains("target_dir"), "{text}");
        assert!(text.contains("live_dir"), "{text}");
    }

    #[tokio::test]
    async fn products_select_plugin_keeps_sandbox() {
        let s = state();
        let rows = crate::boxes::products::discover(&s.repo_root);
        if let Some(plugin) = rows.iter().find(|r| r.role == "plugin") {
            let sel = rpc(
                &s,
                110,
                "tools/call",
                json!({ "name": "gsv_products_select", "arguments": { "id": plugin.id } }),
            )
            .await;
            assert_eq!(sel["result"]["isError"], false, "{sel}");
        }
        ...
        assert!(
            sandbox.replace('\\', "/").ends_with("/GSV"),
            "sandbox must stay the GSV crate after plugin select, got {sandbox}"
        );
        assert!(!sandbox.to_ascii_lowercase().ends_with("/poolai"));
    }
