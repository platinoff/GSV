...
async fn run_smokes(cli: &Cli) -> SmokeReport {
    ...
    record(
        &mut cases,
        "watchdog",
        check_ok(&client, &cli.base_url, "/api/watchdog"),
    )
    .await;
    record(
        &mut cases,
        "usage",
        check_ok(&client, &cli.base_url, "/api/usage"),
    )
    .await;
    record(
        &mut cases,
    ...
    record(
        &mut cases,
        "tickets",
        check_ok(&client, &cli.base_url, "/api/tickets"),
    )
    .await;
    record(
        &mut cases,
        "tickets-bench",
        check_ok(&client, &cli.base_url, "/api/tickets/bench"),
    )
    .await;
    record(
        &mut cases,
    ...
    record(
        &mut cases,
        "ui_icons_svg",
        check_status(&client, &cli.base_url, "/api/ui/icons.svg"),
    )
    .await;
    record(
        &mut cases,
        "ui_icon_about",
        check_status(&client, &cli.base_url, "/api/ui/icon/about.svg"),
    )
    .await;
