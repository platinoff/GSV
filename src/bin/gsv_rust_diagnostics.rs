...
//!
//! Canonical JSON: [`docs/vision/rust_diagnostics.json`](../../docs/vision/rust_diagnostics.json)
//!
//! ```text
//! cargo run --bin gsv-rust-diagnostics -- --print
//! cargo run --bin gsv-rust-diagnostics -- --scan
//! cargo run --bin gsv-rust-diagnostics -- --record --warnings 0 --errors 0 --ok
//! cargo run --bin gsv-rust-diagnostics -- --from-json path/to/cargo.json
//! ```
...
const DEFAULT_OUTPUT: &str = "docs/vision/rust_diagnostics.json";
const HISTORY_CAP: usize = 32;
const DEFAULT_SCAN_CMD: &str = "cargo clippy --message-format=json --all-targets";
...
fn usage() {
    eprintln!(
        "Usage:
  gsv-rust-diagnostics --print [--output PATH]
  gsv-rust-diagnostics --scan [--command CMD] [--host LABEL] [--source local|ci] [--output PATH]
  gsv-rust-diagnostics --from-json FILE [--command CMD] [--host LABEL] [--source local|ci] [--output PATH]
  gsv-rust-diagnostics --record --warnings N --errors N [--ok|--fail] [--command CMD] [--host LABEL] [--source local|ci] [--wall-secs SECS] [--output PATH]"
    );
}

fn persist(output: &Path, index: &RustDiagnostics) -> ExitCode {
    if let Err(e) = write_index(output, index) {
        eprintln!("write failed: {e}");
        return ExitCode::FAILURE;
    }
    print_summary(index);
    if index.latest.errors.unwrap_or(0) > 0 {
        ExitCode::FAILURE
    } else {
        ExitCode::SUCCESS
    }
